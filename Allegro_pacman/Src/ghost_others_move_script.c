...
static void ghost_random_move_script_FREEDOM(Ghost* ghost, Map* M, Pacman* pacman);
static void ghost_random_move_script_BLOCKED(Ghost* ghost, Map* M, Pacman* pacman);
...
static void ghost_random_move_script_FREEDOM(Ghost* ghost, Map* M, Pacman* pacman) {
	...
	int targetX = generateRandomNumber(1, 34);
	int targetY = generateRandomNumber(1, 28);


	if ( M->map[targetY][targetX] != '#') {
		game_log("Altered\n");
		ghost->objData.target.x = targetX;
		ghost->objData.target.y = targetY;
	}

	printf("Ghost target movement is %d %d\n", ghost->objData.target.x, ghost->objData.target.y);
	if (ghost->objData.target.x > 0 && ghost->objData.target.y > 0) {
		Directions shortestPath = shortest_path_direc(M, ghost->objData.Coord.x, ghost->objData.Coord.y, ghost->objData.target.x, ghost->objData.target.y);
		ghost_NextMove(ghost, shortestPath);
	}

	
	if (ghost->objData.Coord.x == ghost->objData.target.x && ghost->objData.Coord.y == ghost->objData.target.y) {
		ghost->objData.target.x = -1;
		ghost->objData.target.y = -1;
	}
	...
	*/
	/*
	if (ghost->objData.target.x == NULL && ghost->objData.target.y==NULL) {
		ghost->objData.target.x = generateRandomNumber(1, 34);
		ghost->objData.target.y = generateRandomNumber(1, 28);
	}
	*/
	

	//printf("Ghost target data is %d %d\n", ghost->objData.target.x, ghost->objData.target.y);
...
void ghost_random_move_script(Ghost* ghost, Map* M, Pacman* pacman) {
	if (!movetime(ghost->speed))
		return;
	switch (ghost->status)
	{
	case BLOCKED:
		ghost_random_move_script_BLOCKED(ghost, M, pacman);
		if (al_get_timer_count(game_tick_timer) > GO_OUT_TIME) {
			ghost->status = GO_OUT;
		}
		break;
	case FREEDOM:
		ghost_random_move_script_FREEDOM(ghost, M, pacman);
		break;
	case GO_OUT:
